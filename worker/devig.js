...
// Returns null for zero-margin markets (booksum ≤ 1) since Shin requires a margin to remove.
export function shinFairProbs(prices) {
  ...
  if (booksum <= 1) return null; // Shin only: requires a margin to remove
  ...
  const total = a + b;
  // Validate that bisection converged to a valid Shin root within tolerance
