...
> **⚠ SUPERSEDED (2026-09-19): architecture pivot.** Tasks 1-5 below were fully implemented and reviewed clean, but the design they built — the Worker calling Anthropic's Messages API directly via `worker/pickGenerator.js` — was rejected by the user after the fact: they will not enable pay-per-token Anthropic API billing. `worker/oddsApi.js` and `worker/pickGenerator.js` have since been **deleted**, and `handleGenerateAndPost`/`CRON_SLOTS` in `worker/index.js` have been **replaced** by a new `handlePostSlot`/`POST /api/admin/post-slot` endpoint (see commit `3d5ae29`). Generation now happens in an external **Claude Code cloud routine** (billed against the user's Claude Code plan, not Anthropic API credits), which will fetch odds and generate picks itself, POST each pick to the existing `/api/admin/picks`, then POST to `/api/admin/post-slot` to trigger that slot's tweet.
>
> **What's still true below:** Task 1 (schema: `picks.slot`, `daily_posts` composite `(date, slot)` key) and Task 2 (`db.js` slot support) — both shipped and unaffected by the pivot.
> **What's obsolete:** Tasks 3, 4, 6, and 7's Anthropic-API-key steps — no longer needed, there is no more direct API call to key.
> **What's superseded:** Task 5's `handleGenerateAndPost`/`CRON_SLOTS` design — replaced by `handlePostSlot`, see commit `3d5ae29`.
> **Still pending:** designing and creating the actual Claude Code cloud routine (via the `schedule` skill), and production deployment of the surviving pieces (Task 1/2's migration + the new `post-slot` endpoint).
>
> Full rationale and scope: `.superpowers/sdd/2026-09-19-ai-pick-generation/progress.md` (§ "Architecture pivot: Anthropic API billing rejected"). The task text below is left as-is as a historical record of what was built and then removed — do not re-implement Tasks 3/4/6/7's Anthropic API steps.
